...
async function cadastrar(req, res) {
    try {
        ...
        const emailExistente = await prisma_1.default.usuario.findUnique({ where: { email } });
        if (emailExistente) {
            return res.status(409).json({ erro: "E-mail já cadastrado." });
        }
        // Verifica se CPF já está cadastrado (se fornecido)
        if (cpf) {
            const cpfExistente = await prisma_1.default.usuario.findUnique({ where: { cpf } });
            if (cpfExistente) {
        ...
        const token = jsonwebtoken_1.default.sign({ id: usuario.id, email: usuario.email, role: usuario.role }, process.env.JWT_SECRET, { expiresIn: "7d" });
