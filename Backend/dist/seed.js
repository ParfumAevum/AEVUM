...
async function main() {
    ...
    const categorias = await Promise.all([
        prisma_1.default.categoria.upsert({
            where: { nome: "Parfum" },
            update: {},
            create: { nome: "Parfum" },
        }),
    ...
    // ─── Usuário Admin ──────────────────────────────────────────
    const senhaAdmin = await bcryptjs_1.default.hash("admin123", 10);
    const admin = await prisma_1.default.usuario.upsert({
    ...
    const senhaCliente = await bcryptjs_1.default.hash("cliente123", 10);
    const cliente = await prisma_1.default.usuario.upsert({
    ...
    for (const produto of produtos) {
        await prisma_1.default.produto.upsert({
